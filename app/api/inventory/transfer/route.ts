...
import { processTransferServer } from "@/lib/services/server/stockMutationService";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const phase = body.phase === "receive" ? "receive" : "dispatch";
    ...
    const result = await processTransferServer({
      barcodeId,
      phase,
      fromGodownId: fromGodownId || undefined,
      toGodownId,
      handledBy,
    });
